...
def handle_users():
    ...
    if request.method == 'GET':
        ...
        if users:
            for row in users:
                user, teacher, student = row  # Destructuración de Python
                data = user.serialize()
...
def handle_user(id):
    ...
    print(id)
    if request.method == 'GET':
        user = db.session.execute(db.select(Users, Teachers, Students)
                                            .join(Teachers, Users.id == Teachers.user_id, isouter=True)
                                            .join(Students, Users.id == Students.user_id, isouter=True)
                                            .where(Users.id == id)).first()
        if user:
            user, teacher, student = user
            results = user.serialize()
            results['profile'] = teacher.serialize() if user.role == 'Teacher' else student.serialize() if user.role == 'Student' else {}
        print(results)
        response_body['message'] = 'Metodo GET de users'
        response_body['results'] = results
        return response_body, 200
